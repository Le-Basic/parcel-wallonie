export const FAQ_CONTENT = [
  {
    titreCategorie:
      "Les questions sur… la méthode, le périmètre et les données",
    questions: [
      {
        titreQuestion:
          "La base de données des entités géographiques (communes, provinces, arrondissements, GALs et Parcs naturels, …) utilisée par PARCEL-Wallonie est-elle à jour ?",
        reponseQuestion:
      ...
      },
      {
        ...
        reponseQuestion:
          "<section><p>Quel que soit le choix de périmètre géographique de l’utilisateur, PARCEL-Wallonie propose de relocaliser l’ensemble des produits. Ainsi, il peut arriver que PARCEL-Wallonie propose de relocaliser des produits qui actuellement ne sont pas produits sur le territoire en question.</p><p>Le choix a été fait de proposer une relocalisation pour l’ensemble des produits, quel que soit le territoire. PARCEL-Wallonie fournit donc des estimations théoriques, sans aborder directement la nécessité d’une réorganisation des filières.</p><p>Cette réflexion sur une éventuelle réorganisation devra être menée dans un second temps pour comprendre les raisons de l’absence de certaines productions (pédoclimatiques, économiques, politiques, etc.) à une échelle donnée et les implications socio-économiques d’un tel changement.</p></section>",
      },
    ],
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        reponseQuestion:
          "<section><p>Pour le régime actuel, PARCEL-Wallonie utilise la consommation moyenne de chaque habitant, en se basant sur la consommation globale de l’ensemble de la population Belge pour chacun des 40 produits bruts répertoriés dans l’outil. Ces consommations par produit sont estimées à l’aide de diverses sources :</p><ul><li>Les bilans d’approvisionnement de Statbel</li><li>Des bilans d’approvisionnement reconstitués à partir des données de production Statbel, d’importation et exportation Comtrade</li><li>Pour les céréales, PARCEL-Wallonie reprend les résultats de consommation de PARCEL France</li></ul><p>Ces bilans prennent en compte les gaspillages et les pertes au sein des filières.</p><p>Grâce aux bilans d’approvisionnement, PARCEL-Wallonie estime des volumes globaux de matières premières agricoles nécessaires à l’alimentation sans avoir besoin de prendre en compte la multitude de produits transformés (pizzas, ratatouille…) qui sont consommés chaque année et dont il est difficile de modéliser la recette exacte.</p><p>PARCEL-Wallonie territorialise ensuite les régimes alimentaires à l’échelle des communes en croisant cette donnée de consommation moyenne par habitant avec :</p><ul><li>Les coefficients de variation des régimes alimentaires selon l’âge et le sexe des consommateurs (issues des résultats de l’étude française INCA 3 de l’ANSES publiée en 2018, et adaptée à la distribution démographique Belge.)</li><li>Les données de population des communes (données Statbel)</li></ul><p>Pour plus de détail sur cette partie, se référer la Note méthodologique de l’outil PARCEL, pages 8 à 10</p></section>",
      },
      {
        titreQuestion:
    ...
    questions: [
      {
        ...
        reponseQuestion:
          "<p>Pour l’affichage des surfaces actuelles des territoires, PARCEL-Wallonie utilise les données de Statbel 2022.</p>",
      },
      {
        titreQuestion:
          "Est-ce que PARCEL-Wallonie tient compte des spécificités de production locales (rendements) pour calculer les surfaces nécessaires ?",
        reponseQuestion:
  ...
  },
  {
    ...
    questions: [
      {
        titreQuestion:
          "Comment interpréter les impacts de la simulation, exprimés sous forme de différence avec les impacts que représentent les besoins alimentaires actuels (colonne de gauche) ? ",
        reponseQuestion:
